...
def _select_pivot_context_candles(window: "pd.DataFrame", pivot_index: int) -> dict[str, dict[str, Any]]:
    ...
    pivot_row = window.loc[pivot_index]
    previous_row = window.loc[pivot_index - 1]
    next_row = window.loc[pivot_index + 1]
...
def _is_pivot_high(window: "pd.DataFrame", index: int) -> bool:
    ...
    return bool(
        float(window.loc[index, "close"]) > float(window.loc[index - 1, "close"])
        and float(window.loc[index, "close"]) > float(window.loc[index + 1, "close"])
    )
