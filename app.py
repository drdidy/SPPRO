...
def _resolve_one_intelligence_date(
    trading_date: date,
    prior_date: date,
    effective_offset: float,
) -> bool:
    ...
    try:
        ...
        next_session_spx = build_synthetic_spx_session(
            get_next_day_session_candles(es_candles, trading_date), effective_offset
        )
        ...
        primary_review = evaluate_play_outcome(
            signal_package["scenario"].get("primary_play"), projected_spx, next_session_spx
        )
        alternate_review = evaluate_play_outcome(
            signal_package["scenario"].get("alternate_play"), projected_spx, next_session_spx
        )
        ...
        result_class = (
            chosen["result_classification"]
            if (trade_taken or "invalid_stop" in integrity)
            else "No Trade"
        )
    ...
    for td_str in pending:
        try:
            ...
            with _conn_context() as db:
                sig_row = db.execute(
                    "SELECT prior_date FROM signals WHERE trading_date=?", (td_str,)
                ).fetchone()
            if sig_row is None:
                continue
    ...
    for i, trading_date in enumerate(all_dates):
        ...
        with _conn_context() as db:
            existing_sig = db.execute(
                "SELECT trading_date FROM signals WHERE trading_date=?", (trading_date.isoformat(),)
            ).fetchone()
            existing_out = db.execute(
                "SELECT trading_date FROM outcomes WHERE trading_date=?", (trading_date.isoformat(),)
            ).fetchone()
        if existing_sig is None:
            try:
                ...
                if existing_out is None:
                    primary_review = evaluate_play_outcome(sp["scenario"].get("primary_play"), projected_spx, next_session_spx)
                    alternate_review = evaluate_play_outcome(sp["scenario"].get("alternate_play"), projected_spx, next_session_spx)
                    trade_taken = bool(primary_review["entry_triggered"] or alternate_review["entry_triggered"])
                    chosen = primary_review if primary_review["entry_triggered"] else alternate_review
                    chosen_path = "Primary" if primary_review["entry_triggered"] else ("Alternate" if alternate_review["entry_triggered"] else "None")
...
def render_intelligence_tab(effective_offset: float, inputs: dict[str, Any]) -> None:
    ...
    _chip_row = (
        ...
        f'<div style="padding:8px 16px;border-radius:10px;background:rgba(130,80,255,0.10);'
        f'border:1px solid rgba(130,80,255,0.22);font-size:0.78rem;color:rgba(200,170,255,0.9);">'
        f'<span style="font-weight:700;font-size:1.05rem;color:#c8a8ff;">{sig_count}</span>&nbsp; Signals Logged</div>'
        f'<div style="padding:8px 16px;border-radius:10px;background:rgba(0,212,100,0.08);'
        f'border:1px solid rgba(0,212,100,0.20);font-size:0.78rem;color:rgba(140,240,180,0.9);">'
        f'<span style="font-weight:700;font-size:1.05rem;color:#43f3a3;">{out_count}</span>&nbsp; Outcomes Resolved</div>'
        f'<div style="padding:8px 16px;border-radius:10px;background:rgba(255,180,50,0.07);'
        f'border:1px solid rgba(255,180,50,0.18);font-size:0.78rem;color:rgba(255,210,120,0.9);">'
        f'<span style="font-weight:700;font-size:1.05rem;color:#ffd060;">{pending_count}</span>&nbsp; Pending</div>'
        f'<div style="padding:8px 16px;border-radius:10px;background:rgba(100,150,255,0.07);'
        f'border:1px solid rgba(100,150,255,0.18);font-size:0.78rem;color:rgba(160,195,255,0.9);">'
    ...
    # ── Backfill controls ──────────────────────────────────────────────────────
    with st.expander("Historical Backfill", expanded=(sig_count == 0)):
        st.markdown(
            '<div style="font-size:0.82rem;color:rgba(180,205,240,0.60);margin-bottom:12px;line-height:1.6;">'
            'Run a one-time (or periodic) backfill to populate the intelligence DB with historical signals and outcomes. '
            'Yahoo Finance provides daily + 1h ES=F data covering ~2 years of history. '
            'The engine uses the same projection infrastructure as the live app.'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        with bf_col1:
            ...
            bf_start = st.date_input("Backfill from (next-trading-day)", value=default_start, key="intel_bf_start")
        with bf_col2:
            bf_end = st.date_input("Backfill to (next-trading-day)", value=date.today() - timedelta(days=1), key="intel_bf_end")
        ...
        if st.button("Run Backfill", use_container_width=True, key="intel_run_backfill", type="primary"):
            if bf_end < bf_start:
                ...
                prog_ph = st.empty()
                with st.spinner("Running backfill — this may take several minutes for a full year of data..."):
                    attempted, succeeded = run_intelligence_backfill(
                        start_date=bf_start,
                        end_date=bf_end,
                        effective_offset=effective_offset,
    ...
    st.markdown(
        '<div style="height:1px;background:linear-gradient(90deg,transparent,rgba(130,80,255,0.15),transparent);'
        'margin:8px 0 24px 0;"></div>',
        unsafe_allow_html=True,
    )
    ...
    overall_pnl = overall.get("avg_pnl")
    overall_total = overall.get("total_pnl")

    st.markdown(
        '<div style="font-family:\'Outfit\',sans-serif;font-size:0.95rem;font-weight:800;'
        'color:#e8f2ff;margin-bottom:14px;">◆ Overall Performance</div>',
        unsafe_allow_html=True,
    )
    ...
    with ov_cols[1]:
        st.metric("Win Rate", f"{overall_wr * 100:.1f}%" if overall_wr is not None else "—")
    with ov_cols[2]:
        st.metric("Avg P&L / Session", f"{overall_pnl:+.1f} pts" if overall_pnl is not None else "—")
    with ov_cols[3]:
        st.metric("Total Accumulated P&L", f"{overall_total:+.1f} pts" if overall_total is not None else "—")
    ...
    st.markdown(
        '<div style="font-family:\'Outfit\',sans-serif;font-size:0.95rem;font-weight:800;'
        'color:#e8f2ff;margin-bottom:14px;">◆ Edge by Scenario</div>',
        unsafe_allow_html=True,
    )
    by_sc = stats.get("by_scenario", {})
    if by_sc:
        _sc_rows = []
        for sc_name, sc_stat in sorted(by_sc.items(), key=lambda x: -(x[1].get("total_pnl") or 0)):
            wr = sc_stat.get("win_rate")
            ap = sc_stat.get("avg_pnl")
            tp = sc_stat.get("total_pnl")
            _sc_rows.append({
                "Scenario": sc_name,
                "Sessions": sc_stat["n"],
                "Win Rate": f"{wr * 100:.1f}%" if wr is not None else "—",
                "Avg P&L": f"{ap:+.1f}" if ap is not None else "—",
                "Total P&L": f"{tp:+.1f}" if tp is not None else "—",
            })
        if _sc_rows:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Avg P&L": st.column_config.TextColumn("Avg P&L (pts)", width="small"),
                    "Total P&L": st.column_config.TextColumn("Total P&L (pts)", width="small"),
                },
            )
    ...
    # ── Direction + Confirmation side-by-side ──────────────────────────────────
    dir_col, conf_col = st.columns(2, gap="large")

    with dir_col:
        st.markdown(
            '<div style="font-family:\'Outfit\',sans-serif;font-size:0.95rem;font-weight:800;'
            'color:#e8f2ff;margin-bottom:14px;">◆ Direction Intelligence</div>',
            unsafe_allow_html=True,
        )
        by_dir = stats.get("by_direction", {})
        for d_name, d_stat in by_dir.items():
            wr = d_stat.get("win_rate")
            ap = d_stat.get("avg_pnl")
            n = d_stat["n"]
            _accent = "#43f3a3" if d_name == "CALL" else "#ff6d8b"
            st.markdown(
                f'<div style="background:rgba(255,255,255,0.025);border:1px solid rgba(255,255,255,0.07);'
                f'border-left:3px solid {_accent};border-radius:10px;padding:12px 14px;margin-bottom:10px;">'
                f'<div style="display:flex;justify-content:space-between;align-items:center;">'
                f'<span style="font-size:0.85rem;font-weight:700;color:{_accent};">{d_name}</span>'
                f'<span style="font-size:0.72rem;color:rgba(180,210,240,0.45);">{n} sessions</span></div>'
                f'<div style="display:flex;gap:20px;margin-top:8px;">'
                f'<div><div style="font-size:0.6rem;color:rgba(155,185,220,0.45);font-weight:600;letter-spacing:0.08em;">WIN RATE</div>'
                f'<div style="font-size:1.0rem;font-weight:700;color:#e8f2ff;">{f"{wr*100:.1f}%" if wr is not None else "—"}</div></div>'
                f'<div><div style="font-size:0.6rem;color:rgba(155,185,220,0.45);font-weight:600;letter-spacing:0.08em;">AVG P&L</div>'
                f'<div style="font-size:1.0rem;font-weight:700;color:#e8f2ff;">{f"{ap:+.1f} pts" if ap is not None else "—"}</div></div>'
                f'</div></div>',
                unsafe_allow_html=True,
            )
    ...
    with conf_col:
        st.markdown(
            '<div style="font-family:\'Outfit\',sans-serif;font-size:0.95rem;font-weight:800;'
            'color:#e8f2ff;margin-bottom:14px;">◆ Confirmation Signal</div>',
            unsafe_allow_html=True,
        )
        by_conf = stats.get("by_confirmation", {})
        _conf_accent = {"Confirmed": "#43f3a3", "Failed": "#ff6d8b", "Not Recorded": "rgba(180,205,240,0.45)"}
        for c_name, c_stat in by_conf.items():
            wr = c_stat.get("win_rate")
            ap = c_stat.get("avg_pnl")
            n = c_stat["n"]
            _acc = _conf_accent.get(c_name, "rgba(180,205,240,0.45)")
            st.markdown(
                f'<div style="background:rgba(255,255,255,0.025);border:1px solid rgba(255,255,255,0.07);'
                f'border-left:3px solid {_acc};border-radius:10px;padding:12px 14px;margin-bottom:10px;">'
                f'<div style="display:flex;justify-content:space-between;align-items:center;">'
                f'<span style="font-size:0.85rem;font-weight:700;color:{_acc};">{c_name}</span>'
                f'<span style="font-size:0.72rem;color:rgba(180,210,240,0.45);">{n} sessions</span></div>'
                f'<div style="display:flex;gap:20px;margin-top:8px;">'
                f'<div><div style="font-size:0.6rem;color:rgba(155,185,220,0.45);font-weight:600;letter-spacing:0.08em;">WIN RATE</div>'
                f'<div style="font-size:1.0rem;font-weight:700;color:#e8f2ff;">{f"{wr*100:.1f}%" if wr is not None else "—"}</div></div>'
    ...
    st.markdown(
        '<div style="font-family:\'Outfit\',sans-serif;font-size:0.95rem;font-weight:800;'
        'color:#e8f2ff;margin-bottom:14px;">◆ AI Insights</div>',
        unsafe_allow_html=True,
    )
    insights = []
    if by_sc:
        ...
        if best_sc[0] and (best_sc[1].get("total_pnl") or 0) > 0:
            insights.append(
                f"✦ <b>{best_sc[0]}</b> is your highest-earning scenario — "
                f"{best_sc[1]['n']} sessions, "
                f"{(best_sc[1].get('win_rate') or 0)*100:.0f}% win rate, "
                f"+{best_sc[1].get('total_pnl', 0):.1f} pts total."
            )
        if best_wr_sc[0] and (best_wr_sc[1].get("win_rate") or 0) >= 0.6:
            insights.append(
                f"✦ <b>{best_wr_sc[0]}</b> has the highest win rate at "
                f"{(best_wr_sc[1].get('win_rate') or 0)*100:.1f}% over {best_wr_sc[1]['n']} sessions."
            )
        if worst_sc[0] and (worst_sc[1].get("total_pnl") or 0) < 0:
            insights.append(
                f"✦ Exercise caution with <b>{worst_sc[0]}</b> — "
                f"it has underperformed across {worst_sc[1]['n']} sessions "
                f"({(worst_sc[1].get('win_rate') or 0)*100:.0f}% win rate)."
            )
    if by_dir:
        best_dir = max(by_dir.items(), key=lambda x: x[1].get("win_rate") or 0, default=(None, {}))
        if best_dir[0]:
            insights.append(
                f"✦ <b>{best_dir[0]}</b> trades have historically outperformed with a "
                f"{(best_dir[1].get('win_rate') or 0)*100:.1f}% win rate."
            )
    if by_conf:
        conf_stat = by_conf.get("Confirmed", {})
        fail_stat = by_conf.get("Failed", {})
        if conf_stat.get("win_rate") and fail_stat.get("win_rate"):
            delta = (conf_stat["win_rate"] - fail_stat["win_rate"]) * 100
            if delta > 5:
                insights.append(
                    f"✦ Confirmation adds meaningful edge — confirmed setups win "
                    f"{delta:.0f}% more often than failed confirmation setups."
                )
    if not insights:
    ...
    for ins in insights:
        st.markdown(
            f'<div style="background:rgba(130,80,255,0.06);border:1px solid rgba(130,80,255,0.18);'
            f'border-left:3px solid rgba(160,100,255,0.7);border-radius:10px;padding:12px 16px;'
            f'margin-bottom:8px;font-size:0.82rem;color:rgba(200,180,255,0.85);line-height:1.6;">'
            f'{ins}</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        '<div style="font-family:\'Outfit\',sans-serif;font-size:0.95rem;font-weight:800;'
        'color:#e8f2ff;margin-bottom:14px;">◆ Recent Signal Log</div>',
        unsafe_allow_html=True,
    )
    all_recs = _intelligence.get_all_records()
    log_rows = []
    for r in all_recs[:60]:
        log_rows.append({
            "Date": r["trading_date"],
            "Scenario": (r.get("scenario_name") or "—"),
            "Direction": r.get("primary_direction") or "—",
            "Confirmation": r.get("confirmation_status") or "—",
            "Sit-Out": "Yes" if r.get("sit_out") else "No",
            "Result": r.get("result_classification") or "Pending",
            "P&L (pts)": f"{r['primary_pnl']:+.1f}" if r.get("primary_pnl") is not None else "—",
            "Outcome": "✓" if r.get("primary_tp1_hit") else ("✗" if r.get("primary_stop_hit") else "·"),
            "Source": "Backfill" if r.get("is_backfill") else "Live",
        })
    if log_rows:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Sit-Out": st.column_config.TextColumn("Sit-Out", width="small"),
                "Result": st.column_config.TextColumn("Result", width="medium"),
                "P&L (pts)": st.column_config.TextColumn("P&L", width="small"),
...
def main() -> None:
    ...
    if (
        _intelligence is not None
        and signal_package is not None
        and inputs.get("operating_mode") == "Live Mode"
    ):
        try:
