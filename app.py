...
try:
    from core.data_fetch import (
        ...
        fetch_es_hourly_candles_with_diagnostics,
...
try:
    ...
    PROVIDER_NAMES = ["none"]
    OptionLookupRequest = None
    load_options_provider = None
...
def raise_startup_import_error() -> None:
    """Raise a clear startup error when required app imports are unavailable."""

    messages = [message for message in [STREAMLIT_IMPORT_ERROR, CORE_IMPORT_ERROR, OPTIONS_IMPORT_ERROR] if message]
    if messages:
...
def resolve_anchor_bundle(inputs: dict[str, Any]) -> tuple[dict[str, Any], Any | None, str | None, dict[str, Any] | None]:
    ...
    try:
        es_candles, base_diagnostics = fetch_es_hourly_candles_with_diagnostics(
        ...
        diagnostics["fetch_error"] = diagnostics.get("fetch_error") or str(exc)
        ...
        return (
            ...
            es_candles,
            diagnostics.get("explicit_error_message_if_dataframe_is_empty") or str(exc),
            diagnostics,
        )
...
def main() -> None:
    ...
    if data_error:
        st.warning(f"Auto-fetch failed because Yahoo returned no usable intraday ES=F data. Manual anchors are being used. Details: {data_error}")
