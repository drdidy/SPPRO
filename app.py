...
from core.data_fetch import ES_FUTURES_SYMBOL, extract_spx_830_candle, fetch_es_hourly_candles, fetch_spx_confirmation_candles
...
from core.time_utils import at_central, build_session_windows, current_central_time, filter_time_range, market_time_to_central
...
def build_es_fetch_request_params(prior_session_date: date, next_trading_date: date) -> dict[str, Any]:
    """Mirror the ES auto-fetch request parameters for diagnostics."""

    start = at_central(prior_session_date, 8, 30) - timedelta(days=2)
    end = at_central(next_trading_date, 19, 0) + timedelta(hours=1)
    return {
        "tickers": ES_FUTURES_SYMBOL,
        "interval": "60m",
...
def _normalize_diagnostic_price_frame(raw_frame: Any) -> pd.DataFrame:
    """Normalize a raw yfinance frame for diagnostics only."""
...
def collect_auto_fetch_diagnostics(prior_session_date: date, next_trading_date: date) -> dict[str, Any]:
    """Collect raw fetch diagnostics without changing engine behavior."""

    diagnostics: dict[str, Any] = {
        "mode": "Auto-fetch",
        "raw_ticker_used": ES_FUTURES_SYMBOL,
        "raw_yfinance_request_parameters": build_es_fetch_request_params(prior_session_date, next_trading_date),
        ...
        "row_count_in_full_ny_session_filter": 0,
        "row_count_in_12_pm_to_4_pm_ct_afternoon_filter": 0,
        "row_count_in_overnight_filter": 0,
        "pivot_high_found": False,
        "pivot_low_found": False,
        "session_extremes_found": False,
        "explicit_error_message_if_dataframe_is_empty": None,
        "fetch_error": None,
    }

    request_params = diagnostics["raw_yfinance_request_parameters"]
    ...
    try:
        ...
        raw_frame = yf.download(
            ...
            auto_adjust=request_params["auto_adjust"],
            prepost=request_params["prepost"],
        )
    except Exception as exc:
        diagnostics["fetch_error"] = str(exc)
        diagnostics["explicit_error_message_if_dataframe_is_empty"] = f"Auto-fetch request failed before normalization: {exc}"
        return diagnostics

    diagnostics["row_count_returned_before_any_filtering"] = int(len(raw_frame))

    if hasattr(raw_frame, "index") and len(raw_frame.index) > 0:
        first_timestamp = raw_frame.index[0]
        last_timestamp = raw_frame.index[-1]
        diagnostics["first_timestamp_returned"] = str(first_timestamp)
        diagnostics["last_timestamp_returned"] = str(last_timestamp)
        timezone = getattr(raw_frame.index, "tz", None)
        diagnostics["timezone_info_before_conversion"] = str(timezone) if timezone is not None else "naive"
    else:
        diagnostics["explicit_error_message_if_dataframe_is_empty"] = "yfinance returned an empty dataframe before normalization."
        diagnostics["timezone_info_before_conversion"] = "unavailable"

    normalized = _normalize_diagnostic_price_frame(raw_frame)
    diagnostics["row_count_after_timezone_conversion"] = int(len(normalized))

    if normalized.empty:
        diagnostics["explicit_error_message_if_dataframe_is_empty"] = (
            diagnostics["explicit_error_message_if_dataframe_is_empty"]
            or "The dataframe is empty after timezone conversion and normalization."
        )
        return diagnostics
    ...
    diagnostics["row_count_in_full_ny_session_filter"] = int(len(ny_session))
    diagnostics["row_count_in_12_pm_to_4_pm_ct_afternoon_filter"] = int(len(afternoon))
    diagnostics["row_count_in_overnight_filter"] = int(len(overnight))

    if len(afternoon) >= 3:
        closes = afternoon["close"].astype(float).tolist()
        diagnostics["pivot_high_found"] = any(
            closes[index] > closes[index - 1] and closes[index] > closes[index + 1]
            for index in range(1, len(closes) - 1)
        )
        diagnostics["pivot_low_found"] = any(
            closes[index] < closes[index - 1] and closes[index] < closes[index + 1]
            for index in range(1, len(closes) - 1)
        )
    ...
    if not ny_session.empty:
        ...
        diagnostics["session_extremes_found"] = red_exists and green_exists
...
def resolve_anchor_bundle(inputs: dict[str, Any]) -> tuple[dict[str, Any], Any | None, str | None, dict[str, Any] | None]:
    ...
    diagnostics = collect_auto_fetch_diagnostics(inputs["prior_session_date"], inputs["next_trading_date"])

    try:
        es_candles = fetch_es_hourly_candles(inputs["prior_session_date"], inputs["next_trading_date"])
...
def main() -> None:
    ...
    if data_error:
        st.warning(f"Auto-fetch failed. Manual anchors are being used. Details: {data_error}")
